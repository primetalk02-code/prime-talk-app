...
function TeacherHistory() {
  ...
  const loadHistory = useCallback(async (silent = false) => {
    try {
      ...
      const { data, error: historyError } = await supabase
        .from('lessons')
        .select('id, student_id, status, source, room_url, room_name, duration, textbook, created_at, started_at, ended_at')
  ...
  const handleMarkFinished = async (lessonId) => {
    if (!lessonId || !teacherId || updatingLessonId) {
      return
    }
      ...
      <Card>
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <CardDescription>Review past sessions and manage current lesson states.</CardDescription>
          </div>
          <Button variant="secondary" onClick={() => void loadHistory(true)} disabled={loading || refreshing}>
                ...
                return (
                  ...
                  >
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                          Student: {String(lesson.student_id).slice(0, 8)} | Source: {lesson.source}
                        </p>
                        <p className="mt-1 text-xs text-slate-500">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      <Button size="sm" onClick={() => navigate(`/lesson/${lesson.id}`)} disabled={!canJoin}>
                        Join Lesson
                      </Button>
                      <Button
                        size="sm"
                      ...
                      <Button size="sm" variant="secondary" onClick={() => navigate('/teacher/messages')}>
                        Message Student
                      </Button>
                    </div>
                  </article>

  ...
  return React.createElement('div', { style: { width: '100vw', height: '100vh', display: 'flex', flexDirection: 'column', background: '#F8FAFC', fontFamily: 'system-ui, sans-serif', overflow: 'hidden' } },
    ...
    React.createElement('div', { style: { height: '52px', flexShrink: 0, background: '#0F172A', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px', zIndex: 10 } },
      ...
      React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '16px' } },
        ...
        timer > 0 && React.createElement('div', { style: { background: timer < 300 ? '#EF4444' : '#0EA5A0', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '14px', fontWeight: 700, fontVariantNumeric: 'tabular-nums' } }, formatTime(timer))
      ),
    ...
    React.createElement('div', { style: { flex: 1, overflow: 'hidden', position: 'relative' } },
      ...
      // Textbook content
      article && React.createElement('div', { style: { height: '100%', overflowY: 'auto', padding: '24px', paddingBottom: '120px', maxWidth: '800px', margin: '0 auto' } },

        // Article navigation tabs
        articles.length > 1 && React.createElement('div', { style: { display: 'flex', gap: '8px', marginBottom: '20px', flexWrap: 'wrap' } },
          articles.map((a, i) => React.createElement('button', {
            key: i,
            onClick: () => setCurrentArticle(i),
            style: { padding: '6px 14px', borderRadius: '20px', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: 600, background: i === currentArticle ? '#0EA5A0' : '#E2E8F0', color: i === currentArticle ? 'white' : '#64748B' }
          }, 'Lesson ' + (i + 1)))
        ),

        // Level badge
        React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' } },
          React.createElement('span', { style: { background: '#DBEAFE', color: '#1D4ED8', padding: '3px 10px', borderRadius: '20px', fontSize: '12px', fontWeight: 600 } }, article.level),
          React.createElement('span', { style: { color: '#94A3B8', fontSize: '12px' } }, lesson?.textbook || 'English')
        ),

        // Title
        React.createElement('h1', { style: { fontSize: '24px', fontWeight: 800, color: '#0F172A', marginBottom: '20px', lineHeight: 1.3 } }, article.title),
        ...
        React.createElement('div', { style: { background: 'white', borderRadius: '16px', padding: '24px', boxShadow: '0 1px 8px rgba(0,0,0,0.08)', marginBottom: '16px' } },
          article.content.split('\n').map((line, i) =>
            React.createElement('p', { key: i, style: { margin: '0 0 8px 0', lineHeight: 1.8, color: line.startsWith('-') || line.match(/^\d\./) ? '#0F172A' : line === '' ? undefined : '#334155', fontSize: '15px', fontWeight: line.includes(':') && !line.startsWith('-') && line.length < 40 ? 700 : 400 } }, line || React.createElement('br'))
          )
        ),
        ...
        React.createElement('div', { style: { background: 'white', borderRadius: '16px', padding: '20px', boxShadow: '0 1px 8px rgba(0,0,0,0.08)' } },
          React.createElement('button', {
            ...
            style: { background: 'none', border: 'none', cursor: 'pointer', fontSize: '15px', fontWeight: 700, color: '#0F172A', display: 'flex', alignItems: 'center', gap: '8px', padding: 0 }
          }, (showVocab ? 'v' : '>') + ' Key Vocabulary (' + article.vocab.length + ' words)'),
          showVocab && React.createElement('div', { style: { display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '12px' } },
            article.vocab.map(word => React.createElement('span', { key: word, style: { background: '#F0FDFC', color: '#0EA5A0', border: '1px solid #99F6E4', padding: '6px 14px', borderRadius: '20px', fontSize: '14px', fontWeight: 500 } }, word))
          )
        )
      ),

      // Video overlay - bottom right corner
      React.createElement('div', { style: { position: 'absolute', bottom: '16px', right: '16px', width: '200px', height: '150px', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,0.3)', zIndex: 50, border: '2px solid #0EA5A0', background: '#0D1117' } },
        React.createElement('div', { ref: videoContainerRef, style: { width: '100%', height: '100%' } })
      )
    )
  )
}